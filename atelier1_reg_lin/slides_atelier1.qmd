...
- (15–60) **Théorie complète** — modèle, estimation, diagnostics, variables catégorielles, interactions
- (60–90) **Mission 1** — Modèle sur `AmesHousing::ames`
- (90–120) **Mission 2** — Tests & interprétation (RMSE/MAE)
- (120–150) **Compléments** — sélection de variables, transformations, influence
...
## Rappel : régression simple → multiple *(15–*
- Objectif : approximer une relation moyenne entre une réponse $Y$ et des prédicteurs $X$.
- Extension à $p$ prédicteurs : maintenir l’interprétation **ceteris paribus**.
...
## Encodage des variables catégorielles *(40–*
...
```{r}
# Exemple : Neighborhood sur Ames (échantillon pour compacité)
d <- ames |> dplyr::select(Sale_Price, Gr_Liv_Area, Overall_Qual, Neighborhood) |>
 dplyr::mutate(Neighborhood = droplevels(Neighborhood))
m_cat <- lm(Sale_Price ~ Gr_Liv_Area + Overall_Qual + Neighborhood, data=d)
broom::tidy(m_cat) |> dplyr::slice(1:8)
```

---

## Diagnostics essentiels *(47–*
...
## Transformations & non-linéarités *(130–*
...
## Influence & points atypiques *(147–*
